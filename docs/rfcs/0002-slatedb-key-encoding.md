...
| `inline/*` | `table_id, schema_version, …` | Inlined data — RFC 0005 owns six kinds (`schema`, `insert`, `inline_delete`, `file_delete`, the `file_delete_table` existence marker, and the per-chunk `chunk_range` directory). |
