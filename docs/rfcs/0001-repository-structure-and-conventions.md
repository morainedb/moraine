...
| Real object storage | `crates/moraine/tests/object_storage.rs`, via `cargo xtask s3` | Public-API round-trips against an S3-compatible endpoint. `#[ignore]`d by default; the xtask starts a pinned RustFS, creates the bucket, and runs them un-ignored. The same tests point at a real bucket through `MORAINE_S3_ENDPOINT`. | separate required PR job |
