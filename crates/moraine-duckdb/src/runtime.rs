    ...
    AbiError: From<E>,
{
    ...
    // SAFETY: forwarded caller contract.
    unsafe { block_on_cancellable_within(runtime, probe, probe_ctx, future, STATEMENT_DEADLINE) }
}

/// [`block_on_cancellable_in`], with the deadline named so a test can
/// reach one it can wait out.
///
/// # Safety
///
/// As [`block_on_cancellable_in`].
unsafe fn block_on_cancellable_within<T, E>(
    runtime: &Runtime,
    probe: MoraineInterruptProbe,
    probe_ctx: *mut c_void,
    future: impl Future<Output = Result<T, E>>,
    ...
    AbiError: From<E>,
{
    runtime.block_on(async {
        ...
        // the core future is also immediately ready. The deadline comes
        // last, so a call that finishes on the same poll reports its own
        // result rather than the deadline's.
        tokio::select! {
            biased;
            () = probe_fired => Err(AbiError::interrupted()),
            result = future => result.map_err(AbiError::from),
            () = tokio::time::sleep(deadline) => Err(AbiError::timed_out(deadline)),
        }
    })
}
...
mod deadline_tests {
    ...
    use super::{block_on_cancellable_within, new_runtime};
    use crate::{error::codes, logging::allocate_handle_id};

    const DEADLINE: Duration = Duration::from_millis(50);

    /// A call that outruns the statement deadline is given up on rather
    /// than held forever, and reports it as a cancellation so a caller
    /// already handling one handles this.
    #[test]
    fn a_call_that_outruns_the_deadline_is_given_up_on() {
        let runtime = new_runtime(allocate_handle_id(), 2).unwrap();
        // SAFETY: no probe, so `probe_ctx` is never dereferenced.
        let result: Result<(), _> = unsafe {
            block_on_cancellable_within(
                &runtime,
                None,
                std::ptr::null_mut(),
                std::future::pending::<Result<(), crate::error::AbiError>>(),
                DEADLINE,
            )
        };

        let error = result.expect_err("a pending call must hit the deadline");
        assert_eq!(error.code, codes::INTERRUPTED);
        assert!(
            error.message.contains(&format!("{DEADLINE:?}")),
            "the error names the deadline it hit: {}",
            error.message
        );
    }
}
